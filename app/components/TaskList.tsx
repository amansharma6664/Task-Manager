...
export default function TaskList({ token, tasks, onEdit, refresh, refreshKey }: Props) {
  const [editTaskId, setEditTaskId] = useState<string | null>(null);
  const [editData, setEditData] = useState<{ title: string; description: string } | null>(null);

  useEffect(() => {
    // reset local edit state if refreshKey changes
    setEditTaskId(null);
    setEditData(null);
  }, [refreshKey]);
          ...
          >
            ...
            <div className="flex space-x-2">
              <button
                onClick={() => {
                  handleEdit(task);
                  onEdit?.(task._id, task.title, task.description); // call parent onEdit too
                }}
                className="bg-yellow-500 px-3 py-1 rounded text-white hover:bg-yellow-600"
              >
                Edit
